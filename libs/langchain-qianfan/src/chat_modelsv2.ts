...
import {getSDKIns} from "./utils.js";
...
declare interface BaiduWenxinChatInput {
  ...
  /**
   * Qianfan SDK Instance Name. 
   * ChatCompletion | Completions
   */
  qianfanSDKName?: string;
}
  ...
  implements BaiduWenxinChatInput
{
  ...
  constructor(fields?: Partial<BaiduWenxinChatInput> & BaseChatModelParams) {
    ...
    if (fields?.qianfanSecretKey) {
      ...
      const SDKIns = getSDKIns(fields?.qianfanSDKName);

      this.client = new SDKIns({
  ...
  async completionWithRetry(
    ...
    const makeSDKCompletionRequest = async () => {
      // TODO
      const response = await this.client.completions({
    ...
    if (this.client) {
      return this.caller.call(makeSDKCompletionRequest);
    }
  ...
  async *_streamResponseChunks(
    ...
    const getStream = this.client ? this.createSDKStream : this.createWenxinStream;
